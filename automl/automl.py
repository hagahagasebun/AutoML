...
def st_display_table(df: pd.DataFrame):
    ...
    x_col = st.selectbox("グラフのX軸", columns)
...
def main():
    ...
    if choice == 'グラフ表示':
        ...
        #グラフ表示のセレクトボックスを作成
            columns = df.columns.tolist()
            x_col = st.sidebar.selectbox("グラフのX軸", columns)
            ...
            st_display_graph(df=df,x_col='退職')
    ...
    if choice == '学習と検証':
        ...
        if 'df' in st.session_state:
            ...
            st.caption('')
            st.caption('決定木の可視化')
            st_display_dtree(clf, features=train_X.columns.tolist())
